...
GlowShader::GlowShader(const std::string& AssetDirectory) : PhongShader(false), Scaling(1,1,1), MixTex(NULL)
{
    ...
    FactorKLoc = getParameterID( "FactorK");
...
void GlowShader::activate(const BaseCamera& Cam) const
{
    ...
    setParameter(FactorKLoc, FactorK);
}

void GlowShader::deactivate() const
{
...
void GlowShader::activateTex(const Texture* pTex, GLint Loc, int slot) const
{
    if(pTex && Loc>=0)
    {
        pTex->activate(slot);
